...
for(j in 1:nrow(df.full)){
if(is.na(df.full$did_fish_have_a_median_above_60[j]) == TRUE){
 df.full$revisedlearningcategory[j]<-NA}  
else if (df.full$did_fish_have_a_median_above_60[j] == 0){
  df.full$revisedlearningcategory[j]<-"nonlearner"}
else if (df.full$did_fish_have_at_least_two_medians_above_80[j] == 1){
  df.full$revisedlearningcategory[j]<-"highlearner"}
else{
  df.full$revisedlearningcategory[j]<-"learner"}
}
  



df.full$revisedlearningcategory<-as.character(df.full$revisedlearningcategory)
df.full$revisedlearningcategorynumeric<-as.character(df.full$revisedlearningcategory)

for(k in 1:nrow(df.full)){
if (is.na(df.full$revisedlearningcategorynumeric[k]) == TRUE){
  df.full$revisedlearningcategorynumeric[k]<-NA}  
else if (df.full$revisedlearningcategorynumeric[k] == "nonlearner"){df.full$revisedlearningcategorynumeric[k] <-0}
else if (df.full$revisedlearningcategorynumeric[k] == "learner"){df.full$revisedlearningcategorynumeric[k] <-1}
else if (df.full$revisedlearningcategorynumeric[k] == "highlearner"){df.full$revisedlearningcategorynumeric[k] <-2}}

df.full$revisedlearningcategorynumeric<-as.numeric(df.full$revisedlearningcategorynumeric)



df.full.highlearners<-df.full %>% filter(number_of_medians_above_80_using_T30_to_T50_data>=2)
df.full.females.highlearners<-df.full.females %>% filter(number_of_medians_above_80_using_T30_to_T50_data>=2)
df.full.males.highlearners<-df.full.males %>% filter(number_of_medians_above_80_using_T30_to_T50_data>=2)

```


#FIGURE 1A redone december 2019 separate for highlearners USE
```{r, echo=FALSE, message=FALSE, warning=FALSE}
...
nonsolvers = c(6,7)
highsolvers = c(3,3)
numsolvesex<-data.frame(nonsolvers, solvers, highsolvers)
chi<-chisq.test(numsolvesex)
chip<-chi$p.value

ggplot(df.full, aes(revisedlearningcategorynumeric, fill = sex.x))+
...
#FIGURE 1C - relaxed criterion #4 (entered both between 4-9 i.e. made a transit) learning criterion still 50% 3 consecutive times USE THIS
```{r, echo=FALSE, message=FALSE, warning=FALSE}
...
plot2 <- ggplot(df.full.females, aes(average_reaction_time, average_performance_across_all_9_testing_trials_using_T30_to_T50_data)) +
  geom_point(data=df.full.females.highlearners,size = 7, color = "black") +
...
plot3 <- ggplot(df.full.females, aes(unique_zones_entered_full_4_minutes, average_performance_across_all_9_testing_trials_using_T30_to_T50_data)) + geom_point(data=df.full.females.highlearners,size = 7, color = "black") +
  geom_point(size = 4, color = "maroon") + 
  theme(text = element_text(size=10)) + 
  #annotate("text", x=30, y=0.33, parse = TRUE, label= glue::glue("{as.character(rsquaredtxt)}"), size = 5, color = "gray39", hjust = 0) + 
  #annotate("text", x=32, y=0.32, label= glue::glue("= {round(r_squared,4)}"), size = 5, color = "gray39", hjust = 0) + 
...
#n = {number}"), size = 5, color = "gray39", hjust = 0) + 
 ...
 geom_smooth(method = "lm", size = 2, color = "black", fill = "lightgray") +
         ...
         theme(axis.line.x = element_line(color="gray", size = 1))+ 
         theme(panel.background = element_blank()) + 
  theme(plot.title = element_text(hjust = 0))+ scale_x_continuous(name="exploration", limits=c(0.65, 1)) +
  scale_y_continuous(name="numerosity performance", limits=c(0, 0.8)) +
  theme(axis.text.x = element_text(size=25), axis.text.y = element_text(size=25))
plot3
...
plot1 <- ggplot(df.full.females, aes(average_number_of_entries_full_4_minutes, average_performance_across_all_9_testing_trials_using_T30_to_T50_data)) + geom_point(data=df.full.females.highlearners,size = 7, color = "black") + geom_point(size = 4, color = "maroon") + 
  theme(text = element_text(size=10)) + 
  #annotate("text", x=30, y=0.33, parse = TRUE, label= glue::glue("{as.character(rsquaredtxt)}"), size = 5, color = "gray39", hjust = 0) + 
  #annotate("text", x=32, y=0.32, label= glue::glue("= {round(r_squared,4)}"), size = 5, color = "gray39", hjust = 0) + 
  #annotate("text", x=30, y=0.25, label= glue::glue("p = {round(p_value,4)}
...
#PCA of just the three num behaviors, then compared to learning performance looking for sex diferences USE new Fig2D&E
```{r, echo=FALSE, message=FALSE, warning=FALSE}
...
df.full.continuous$did_fish_have_at_least_two_medians_above_80 <-as.factor(df.full.continuous$did_fish_have_at_least_two_medians_above_80)

autoplot(df.full.continuous.pca, data = df.full.continuous, colour = 'sex.x', size = 5, loadings = TRUE, loadings.colour = 'black', shape = "did_fish_have_at_least_two_medians_above_80")+ 
  scale_color_manual(values=c("maroon", "lightblue")) 




fviz_eig(df.full.continuous.pca)
...
df.full.continuous.highlearners<-df.full.continuous %>% filter (number_of_medians_above_80_using_T30_to_T50_data >= 2)
```
...
lm(df.full.continuous.females$average_performance_across_all_9_testing_trials_using_T30_to_T50_data ~ df.full.continuous.females$PC1)
...
plot1 <- ggplot(df.full.continuous, aes(x = PC1, y = average_performance_across_all_9_testing_trials_using_T30_to_T50_data, color = sex.x, linetype =sex.x)) + 
  geom_point(data=df.full.continuous.highlearners,size = 7, color = "black") +
  geom_point(size = 4) + 
...
#New FIG 4A Detour learning by Latency fix
```{r, echo=FALSE, message=FALSE, warning=FALSE}
...
p <- ggplot(df.full, aes(x=detour_trial_1_did_fish_solve_barrier_problem, y=time_to_barrier_contact_minus_start_time)) + 
  geom_boxplot(lwd=1)+ stat_n_text() + theme(axis.text.y = element_text(size = 14)) + theme(axis.text.x = element_text(size = 14)) +
  scale_fill_manual(name="Sex",labels=c("non", "solve"))
...
#New FIG 4B Detour learning by Latency 
```{r, echo=FALSE, message=FALSE, warning=FALSE}
...
p + geom_point(size = 5, color = "black") + 
  theme(text = element_text(size=10)) + 
geom_smooth(method = "lm", size = 2, se = FALSE,full.continuousrange=TRUE, color = "gray") +
  theme(text = element_text(size=10)) +
         theme(axis.text=element_text(size=16),
         axis.title=element_text(size=14))+
         theme(axis.line.y = element_line(color="gray", size = 1))+
...
res <- cor.test(df.full$detour_trial_1_barrier_problem_time_around_barrier_minus_time_to_contact_barrier, df.full$time_to_barrier_contact_minus_start_time, method = "pearson")

```


#SUPP FIG 4B Detour learning by Latency now with quadratic
```{r, echo=FALSE, message=FALSE, warning=FALSE}
#Does latency to approach barrier predict latency to solve barrier problem?

quad<-df.full
quad$time2<-(quad$time_to_barrier_contact_minus_start_time)^2


quadmod<- lm(quad$detour_trial_1_barrier_problem_time_around_barrier_minus_time_to_contact_barrier ~ quad$time_to_barrier_contact_minus_start_time + quad$time2)
summary(quadmod)
...
#old4A males only (Cognitive Style across domains)
```{r, echo=FALSE, message=FALSE, warning=FALSE}
...
#old4B activity in numerosity x average shuttlebox transits during gap interaction
```{r, echo=FALSE, message=FALSE, warning=FALSE}
...
#old4D reaction time in numerosity x latency to aproach barrier in detour interaction
```{r, echo=FALSE, message=FALSE, warning=FALSE}
...
#old4E both sexes barrier motivation by exploration
```{r, echo=FALSE, message=FALSE, warning=FALSE}
...
#running again removing low exploration outlier:
#df.fullexpo_outlier_removed<-df.full %>% filter(unique_zones_entered_full_4_minutes >= 0.5)
#summary(lm(df.fullexpo_outlier_removed$time_to_barrier_contact_minus_start_time ~ df.fullexpo_outlier_removed$unique_zones_entered_full_4_minutes, na.rm = TRUE))

```

#old4F exploration in numerosity x latency to approach barrier in detour interaction
```{r, echo=FALSE, message=FALSE, warning=FALSE}
...
#FIGURE 5 (was fig 3A in original submission) USE
```{r, echo=FALSE, message=FALSE, warning=FALSE}
df.full$detour_trial_1_did_fish_solve_barrier_problem<-as.character(df.full$detour_trial_1_did_fish_solve_barrier_problem)
...
# DEC 2019 - MORE SHUTTLEBOX DATA ANALYSIS old
```{r, echo=FALSE, message=FALSE, warning=FALSE}
...
#FIGURE 1C- UPDATED DECEMBER 2019 no participation criterion old 
```{r, echo=FALSE, message=FALSE, warning=FALSE}
df.full.males$did_fish_learn_OF_SCREEN_TIME_50_percent_within_10_cm_3_consecutive<- as.numeric(df.full.males$did_fish_learn_OF_SCREEN_TIME_50_percent_within_10_cm_3_consecutive)
df.full.females$did_fish_learn_OF_SCREEN_TIME_50_percent_within_10_cm_3_consecutive<- as.numeric(df.full.females$did_fish_learn_OF_SCREEN_TIME_50_percent_within_10_cm_3_consecutive)
...
```
#FIGURE 1C- UPDATED DECEMBER 2019 more relaxed participation criterion - ALSO NOTE USING 50% overall - NOT of screen time old
```{r, echo=FALSE, message=FALSE, warning=FALSE}
...
```
#FIGURE 1C- UPDATED DECEMBER 2019 more relaxed participation criterion - ALSO NOTE USING 80% overall - NOT of screen time (also removed giveup3) old
```{r, echo=FALSE, message=FALSE, warning=FALSE}
...
#FIGURE 2 - NEW DEC 2019 not using
```{r, echo=FALSE, message=FALSE, warning=FALSE}
#REACTION TIME
