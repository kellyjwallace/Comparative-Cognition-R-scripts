...
df.sum5 <- data.frame(fish_name = test_column)
...
df.sum5$fish_name[i] <-fishname
df.sum5$shuttlebox_start_side[i] <-startside
df.sum5$presentation_when_fish_gave_up[i] <-giveup2
df.sum5$shuttlebox_average_distance_from_correct_stimulus_during_stimulus[i]<-mean_distance_from_correct_during_stims
df.sum5$shuttlebox_average_distance_from_correct_stimulus_during_gaps[i]<-mean_distance_from_correct_during_gaps
df.sum5$difference_avg_dist_from_correct_first_5_gaps_minus_last_5_gaps[i] <- comp_5v5_avg_dist #so a more positive number is better
df.sum5$difference_avg_prop_correct_first_5_gaps_minus_last_5_gaps[i] <- comp_5v5_avg_prop_correct #so a more negative number

df.sum5$shuttlebox_slope_using_avg_dist_from_correct[i]<- slope_avg_dist
df.sum5$shuttlebox_wilcox_test_using_avg_dist_from_correct[i]<-wilcox_p_value_avg_dist
df.sum5$binned_shuttlebox_slope_using_avg_dist_from_correct[i]<- binned_slope_avg_dist
df.sum5$binned_shuttlebox_wilcox_test_using_avg_dist_from_correct[i]<-binned_wilcox_p_value_avg_dist

df.sum5$shuttlebox_slope_using_proportion_time_correct_during_gap[i]<- slope_prop_correct
df.sum5$shuttlebox_wilcox_test_using_proportion_time_correct_during_gap[i]<-wilcox_p_value_prop_correct
df.sum5$binned_shuttlebox_slope_using_proportion_time_correct_during_gap[i]<- binned_slope_prop_correct
df.sum5$binned_shuttlebox_wilcox_test_using_proportion_time_correct_during_gap[i]<-binned_wilcox_p_value_prop_correct

df.sum5$shuttlebox_average_proportion_of_time_on_correct_side_during_gaps[i]<- mean_prop_time_correct_during_gap
df.sum5$shuttlebox_average_proportion_of_time_on_correct_side_during_stimulus[i]<- mean_prop_time_correct_during_stim
df.sum5$shuttlebox_proportion_of_time_on_correct_side_during_gaps_slope_across_presentations[i]<-gap_prop_slope
df.sum5$shuttlebox_proportion_of_time_on_correct_side_during_stimulus_slope_across_presentations[i]<-stim_prop_slope

df.sum5$binned_shuttlebox_average_proportion_of_time_on_correct_side_during_gaps[i]<- mean_binned_prop_time_correct_during_gap
df.sum5$binned_shuttlebox_average_proportion_of_time_on_the_correct_side_during_stimulus[i]<- mean_binned_prop_time_correct_during_stim
df.sum5$binned_shuttlebox_proportion_of_time_on_correct_side_during_gaps_slope_across_presentations[i]<- gap_prop_binned_slope
df.sum5$binned_shuttlebox_proportion_of_time_on_correct_side_during_stimulus_slope_across_presentations[i]<- stim_prop_binned_slope
...
#will make a df.sum5.10sec, add that in to the df.total at the end
...
test_column <- vector(length = length(file_list))
df.sum5.10sec <- data.frame(fish_name = test_column)
...
for(i in 1:length(file_list)){
  ...
  df.full
  df.full<-df.full %>% mutate(distance_from_correct = abs(correct_x_pixel - x))
  df.full.gap<-df.full%>%filter(last_10_secs_of_gap == 1) #THIS IS WHAT CHANGES THE WHOLE THING TO FILTER BY LAST 10 SECS
...
#compare first 5 gaps to last 5 gaps
 ...
 #now let's look at the slope of the line of the distance from correct durign the gaps and see if it decreases over time
  ...
  #now let's put it all together
df.sum5.10sec$fish_name[i] <-fishname
df.sum5.10sec$resolution[i] <-resolution
df.sum5.10sec$shuttlebox_start_side[i] <-startside
df.sum5.10sec$presentation_when_fish_gave_up_3_ITI[i] <-giveup2
#df.sum5.10sec$shuttlebox_start_side[i] <-startside
#df.sum5.10sec$presentation_when_fish_gave_up[i] <-giveup2
#df.sum5.10sec$shuttlebox_average_distance_from_correct_stimulus_during_stimulus[i]<-mean_distance_from_correct_during_stims
df.sum5.10sec$last10secdata_shuttlebox_average_distance_from_correct_stimulus_during_gaps[i]<-mean_distance_from_correct_during_gaps
df.sum5.10sec$last10secdata_difference_avg_dist_from_correct_first_5_gaps_minus_last_5_gaps[i] <- comp_5v5_avg_dist #so a more positive number is better
df.sum5.10sec$last10secdata_difference_avg_prop_correct_first_5_gaps_minus_last_5_gaps[i] <- comp_5v5_avg_prop_correct #so a more negative number is better

df.sum5.10sec$last10secdata_shuttlebox_slope_using_avg_dist_from_correct[i]<- slope_avg_dist
df.sum5.10sec$last10secdata_shuttlebox_wilcox_test_using_avg_dist_from_correct[i]<-wilcox_p_value_avg_dist

df.sum5.10sec$last10secdata_shuttlebox_average_proportion_of_time_on_correct_side_during_gaps[i]<- mean_prop_time_correct_during_gap
#df.sum5.10sec$shuttlebox_average_proportion_of_time_on_correct_side_during_stimulus[i]<- mean_prop_time_correct_during_stim
df.sum5.10sec$last10secdata_shuttlebox_proportion_of_time_on_correct_side_during_gaps_slope_across_presentations[i]<-gap_prop_slope
#df.sum5.10sec$shuttlebox_proportion_of_time_on_correct_side_during_stimulus_slope_across_presentations[i]<-stim_prop_slope

df.sum5.10sec$last10secdata_shuttlebox_slope_using_proportion_time_correct_during_gap[i]<- slope_prop_correct
df.sum5.10sec$last10secdata_shuttlebox_wilcox_test_using_proportion_time_correct_during_gap[i]<-wilcox_p_value_prop_correct


}

#setwd("C:/Users/Kelly/Desktop/Grad School/Cummings Lab/Comparative Cognition/Shuttlebox/")
#write_csv(df.sum5.10sec, "shuttlebox_10sec_data_3_ITI_cutoff_comparing_first_and_last_5_june_8_2018.csv")
...
#will make a df.sum5.30sec, add that in to the df.total at the end
#place this after the mutate distance from correct:
...
test_column <- vector(length = length(file_list))
df.sum5.30sec <- data.frame(fish_name = test_column)
...
for(i in 1:length(file_list)){
  ...
  df.full
  df.full<-df.full %>% mutate(distance_from_correct = abs(correct_x_pixel - x))
  df.full.gap<-df.full%>%filter(last_30_secs_of_gap == 1) #THIS IS WHAT CHANGES THE WHOLE THING TO FILTER BY LAST 30 SECS
  
  df.full.stim<-df.full %>% filter(type == 'stim')
  df.full.gap<-df.full.gap %>% filter(type == 'gap')
...
#compare first 3 gaps to last 3 gaps
  giveup2<- as.numeric(giveup2)
  giveup2minus4<-giveup2 - 4
 ...
 #now let's look at the slope of the line of the distance from correct durign the gaps and see if it decreases over time
  ...
  #now let's put it all together
df.sum5.30sec$fish_name[i] <-fishname
df.sum5.30sec$resolution[i] <-resolution
df.sum5.30sec$shuttlebox_start_side[i] <-startside
df.sum5.30sec$presentation_when_fish_gave_up_3_ITI[i] <-giveup2
#df.sum5.30sec$shuttlebox_start_side[i] <-startside
#df.sum5.30sec$presentation_when_fish_gave_up[i] <-giveup2
#df.sum5.30sec$shuttlebox_average_distance_from_correct_stimulus_during_stimulus[i]<-mean_distance_from_correct_during_stims
df.sum5.30sec$last30secdata_shuttlebox_average_distance_from_correct_stimulus_during_gaps[i]<-mean_distance_from_correct_during_gaps
df.sum5.30sec$last30secdata_difference_avg_dist_from_correct_first_5_gaps_minus_last_5_gaps[i] <- comp_5v5_avg_dist #so a more positive number is better
df.sum5.30sec$last30secdata_difference_avg_prop_correct_first_5_gaps_minus_last_5_gaps[i] <- comp_5v5_avg_prop_correct #so a more negative number is better

df.sum5.30sec$last30secdata_shuttlebox_slope_using_avg_dist_from_correct[i]<- slope_avg_dist
df.sum5.30sec$last30secdata_shuttlebox_wilcox_test_using_avg_dist_from_correct[i]<-wilcox_p_value_avg_dist

df.sum5.30sec$last30secdata_shuttlebox_average_proportion_of_time_on_correct_side_during_gaps[i]<- mean_prop_time_correct_during_gap
#df.sum5.30sec$shuttlebox_average_proportion_of_time_on_correct_side_during_stimulus[i]<- mean_prop_time_correct_during_stim
df.sum5.30sec$last30secdata_shuttlebox_proportion_of_time_on_correct_side_during_gaps_slope_across_presentations[i]<-gap_prop_slope
#df.sum5.30sec$shuttlebox_proportion_of_time_on_correct_side_during_stimulus_slope_across_presentations[i]<-stim_prop_slope

df.sum5.30sec$last30secdata_shuttlebox_slope_using_proportion_time_correct_during_gap[i]<- slope_prop_correct
df.sum5.30sec$last30secdata_shuttlebox_wilcox_test_using_proportion_time_correct_during_gap[i]<-wilcox_p_value_prop_correct

}

#setwd("C:/Users/Kelly/Desktop/Grad School/Cummings Lab/Comparative Cognition/Shuttlebox/")
#write_csv(df.sum5.30sec, "shuttlebox_30sec_data_3_ITI_cutoff_june_8_2018.csv")

```

#This is a repeat of the very first chunk, where we're looking across the entire 90 seconds of the gap, but now instead of comparing the first and last 5 ITI's its comparing the first 3 vs the last 3 ITI's
...
test_column <- vector(length = length(file_list))
df.sum3 <- data.frame(fish_name = test_column)
...
#compare first 3 gaps to last 3 gaps
  giveup2<- as.numeric(giveup2)
  giveup2minus2<-giveup2 - 2
  
  if(giveup2 > 5){
    df.full.first3gap<-df.full.gap %>% filter(is.inrange(presentation, range = c(1,3)))
    df.full.last3gap<-df.full.gap %>% filter(is.inrange(presentation, range = c(giveup2minus2, giveup2)))
    df.prop.first3gap<-df.prop.gap %>% filter(is.inrange(presentation, range = c(1,3)))
    df.prop.last3gap<-df.prop.gap %>% filter(is.inrange(presentation, range = c(giveup2minus2, giveup2)))
 ...
 #now let's look at the slope of the line of the distance from correct durign the gaps and see if it decreases over time
  ...
  #now let's put it all together
df.sum3$fish_name[i] <-fishname
df.sum3$shuttlebox_start_side[i] <-startside
df.sum3$presentation_when_fish_gave_up[i] <-giveup2
df.sum3$shuttlebox_average_distance_from_correct_stimulus_during_stimulus[i]<-mean_distance_from_correct_during_stims
df.sum3$shuttlebox_average_distance_from_correct_stimulus_during_gaps[i]<-mean_distance_from_correct_during_gaps
df.sum3$difference_avg_dist_from_correct_first_3_gaps_minus_last_3_gaps[i] <- comp_3v3_avg_dist #so a more positive number is better
df.sum3$difference_avg_prop_correct_first_3_gaps_minus_last_3_gaps[i] <- comp_3v3_avg_prop_correct #so a more negative number

df.sum3$shuttlebox_slope_using_avg_dist_from_correct[i]<- slope_avg_dist
df.sum3$shuttlebox_wilcox_test_using_avg_dist_from_correct[i]<-wilcox_p_value_avg_dist
df.sum3$binned_shuttlebox_slope_using_avg_dist_from_correct[i]<- binned_slope_avg_dist
df.sum3$binned_shuttlebox_wilcox_test_using_avg_dist_from_correct[i]<-binned_wilcox_p_value_avg_dist

df.sum3$shuttlebox_slope_using_proportion_time_correct_during_gap[i]<- slope_prop_correct
df.sum3$shuttlebox_wilcox_test_using_proportion_time_correct_during_gap[i]<-wilcox_p_value_prop_correct
df.sum3$binned_shuttlebox_slope_using_proportion_time_correct_during_gap[i]<- binned_slope_prop_correct
df.sum3$binned_shuttlebox_wilcox_test_using_proportion_time_correct_during_gap[i]<-binned_wilcox_p_value_prop_correct

df.sum3$shuttlebox_average_proportion_of_time_on_correct_side_during_gaps[i]<- mean_prop_time_correct_during_gap
df.sum3$shuttlebox_average_proportion_of_time_on_correct_side_during_stimulus[i]<- mean_prop_time_correct_during_stim
df.sum3$shuttlebox_proportion_of_time_on_correct_side_during_gaps_slope_across_presentations[i]<-gap_prop_slope
df.sum3$shuttlebox_proportion_of_time_on_correct_side_during_stimulus_slope_across_presentations[i]<-stim_prop_slope

df.sum3$binned_shuttlebox_average_proportion_of_time_on_correct_side_during_gaps[i]<- mean_binned_prop_time_correct_during_gap
df.sum3$binned_shuttlebox_average_proportion_of_time_on_the_correct_side_during_stimulus[i]<- mean_binned_prop_time_correct_during_stim
df.sum3$binned_shuttlebox_proportion_of_time_on_correct_side_during_gaps_slope_across_presentations[i]<- gap_prop_binned_slope
df.sum3$binned_shuttlebox_proportion_of_time_on_correct_side_during_stimulus_slope_across_presentations[i]<- stim_prop_binned_slope
...
#REpeat again, now with last 10 seconds and comparing 3v3 first v last
```{r, echo=FALSE, message=FALSE}
...
test_column <- vector(length = length(file_list))
df.sum3.10sec <- data.frame(fish_name = test_column)
...
#compare first 3 gaps to last 3 gaps
 ...
 #now let's look at the slope of the line of the distance from correct durign the gaps and see if it decreases over time
  ...
  #now let's put it all together
df.sum3.10sec$fish_name[i] <-fishname
df.sum3.10sec$shuttlebox_start_side[i] <-startside
df.sum3.10sec$presentation_when_fish_gave_up[i] <-giveup2
#df.sum3.10sec$shuttlebox_average_distance_from_correct_stimulus_during_stimulus[i]<-mean_distance_from_correct_during_stims
df.sum3.10sec$shuttlebox_average_distance_from_correct_stimulus_during_gaps[i]<-mean_distance_from_correct_during_gaps
df.sum3.10sec$difference_avg_dist_from_correct_first_3_gaps_minus_last_3_gaps[i] <- comp_3v3_avg_dist #so a more positive number is better
df.sum3.10sec$difference_avg_prop_correct_first_3_gaps_minus_last_3_gaps[i] <- comp_3v3_avg_prop_correct #so a more negative number

df.sum3.10sec$shuttlebox_slope_using_avg_dist_from_correct[i]<- slope_avg_dist
df.sum3.10sec$shuttlebox_wilcox_test_using_avg_dist_from_correct[i]<-wilcox_p_value_avg_dist
df.sum3.10sec$binned_shuttlebox_slope_using_avg_dist_from_correct[i]<- binned_slope_avg_dist
df.sum3.10sec$binned_shuttlebox_wilcox_test_using_avg_dist_from_correct[i]<-binned_wilcox_p_value_avg_dist

df.sum3.10sec$shuttlebox_slope_using_proportion_time_correct_during_gap[i]<- slope_prop_correct
df.sum3.10sec$shuttlebox_wilcox_test_using_proportion_time_correct_during_gap[i]<-wilcox_p_value_prop_correct
df.sum3.10sec$binned_shuttlebox_slope_using_proportion_time_correct_during_gap[i]<- binned_slope_prop_correct
df.sum3.10sec$binned_shuttlebox_wilcox_test_using_proportion_time_correct_during_gap[i]<-binned_wilcox_p_value_prop_correct

df.sum3.10sec$shuttlebox_average_proportion_of_time_on_correct_side_during_gaps[i]<- mean_prop_time_correct_during_gap
#df.sum3.10sec$shuttlebox_average_proportion_of_time_on_correct_side_during_stimulus[i]<- mean_prop_time_correct_during_stim
df.sum3.10sec$shuttlebox_proportion_of_time_on_correct_side_during_gaps_slope_across_presentations[i]<-gap_prop_slope
df.sum3.10sec$shuttlebox_proportion_of_time_on_correct_side_during_stimulus_slope_across_presentations[i]<-stim_prop_slope

df.sum3.10sec$binned_shuttlebox_average_proportion_of_time_on_correct_side_during_gaps[i]<- mean_binned_prop_time_correct_during_gap
#df.sum3.10sec$binned_shuttlebox_average_proportion_of_time_on_the_correct_side_during_stimulus[i]<- mean_binned_prop_time_correct_during_stim
df.sum3.10sec$binned_shuttlebox_proportion_of_time_on_correct_side_during_gaps_slope_across_presentations[i]<- gap_prop_binned_slope
#df.sum3.10sec$binned_shuttlebox_proportion_of_time_on_correct_side_during_stimulus_slope_across_presentations[i]<- stim_prop_binned_slope
...
#And to round it out, now with last 30 seconds and comparing 3v3 first v last
```{r, echo=FALSE, message=FALSE}
...
test_column <- vector(length = length(file_list))
df.sum3.30sec <- data.frame(fish_name = test_column)
...
#compare first 3 gaps to last 3 gaps
 ...
 #now let's look at the slope of the line of the distance from correct durign the gaps and see if it decreases over time
  ...
  #now let's put it all together
df.sum3.30sec$fish_name[i] <-fishname
df.sum3.30sec$shuttlebox_start_side[i] <-startside
df.sum3.30sec$presentation_when_fish_gave_up[i] <-giveup2
#df.sum3.30sec$shuttlebox_average_distance_from_correct_stimulus_during_stimulus[i]<-mean_distance_from_correct_during_stims
df.sum3.30sec$shuttlebox_average_distance_from_correct_stimulus_during_gaps[i]<-mean_distance_from_correct_during_gaps
df.sum3.30sec$difference_avg_dist_from_correct_first_3_gaps_minus_last_3_gaps[i] <- comp_3v3_avg_dist #so a more positive number is better
df.sum3.30sec$difference_avg_prop_correct_first_3_gaps_minus_last_3_gaps[i] <- comp_3v3_avg_prop_correct #so a more negative number

df.sum3.30sec$shuttlebox_slope_using_avg_dist_from_correct[i]<- slope_avg_dist
df.sum3.30sec$shuttlebox_wilcox_test_using_avg_dist_from_correct[i]<-wilcox_p_value_avg_dist
df.sum3.30sec$binned_shuttlebox_slope_using_avg_dist_from_correct[i]<- binned_slope_avg_dist
df.sum3.30sec$binned_shuttlebox_wilcox_test_using_avg_dist_from_correct[i]<-binned_wilcox_p_value_avg_dist

df.sum3.30sec$shuttlebox_slope_using_proportion_time_correct_during_gap[i]<- slope_prop_correct
df.sum3.30sec$shuttlebox_wilcox_test_using_proportion_time_correct_during_gap[i]<-wilcox_p_value_prop_correct
df.sum3.30sec$binned_shuttlebox_slope_using_proportion_time_correct_during_gap[i]<- binned_slope_prop_correct
df.sum3.30sec$binned_shuttlebox_wilcox_test_using_proportion_time_correct_during_gap[i]<-binned_wilcox_p_value_prop_correct

df.sum3.30sec$shuttlebox_average_proportion_of_time_on_correct_side_during_gaps[i]<- mean_prop_time_correct_during_gap
#df.sum3.30sec$shuttlebox_average_proportion_of_time_on_correct_side_during_stimulus[i]<- mean_prop_time_correct_during_stim
df.sum3.30sec$shuttlebox_proportion_of_time_on_correct_side_during_gaps_slope_across_presentations[i]<-gap_prop_slope
df.sum3.30sec$shuttlebox_proportion_of_time_on_correct_side_during_stimulus_slope_across_presentations[i]<-stim_prop_slope

df.sum3.30sec$binned_shuttlebox_average_proportion_of_time_on_correct_side_during_gaps[i]<- mean_binned_prop_time_correct_during_gap
#df.sum3.30sec$binned_shuttlebox_average_proportion_of_time_on_the_correct_side_during_stimulus[i]<- mean_binned_prop_time_correct_during_stim
df.sum3.30sec$binned_shuttlebox_proportion_of_time_on_correct_side_during_gaps_slope_across_presentations[i]<- gap_prop_binned_slope
#df.sum3.30sec$binned_shuttlebox_proportion_of_time_on_correct_side_during_stimulus_slope_across_presentations[i]<- stim_prop_binned_slope
...
```{r, echo=FALSE, message=FALSE}
setwd("C:/Users/Kelly/Desktop/Grad School/Cummings Lab/Comparative Cognition/Shuttlebox/october analysis")
write_csv(df.sum5, "shuttlebox_data_october_4_2018_all_using_giveup_2_comparing_5_v_5_across_entire_90_second_gap.csv")
write_csv(df.sum5.10sec, "shuttlebox_data_october_4_2018_all_using_giveup_2_comparing_5_v_5_across_across_last_10_second_gap.csv")
write_csv(df.sum5.30sec, "shuttlebox_data_october_4_2018_all_using_giveup_2_comparing_5_v_5_across_across_last_30_second_gap.csv")
write_csv(df.sum3, "shuttlebox_data_october_4_2018_all_using_giveup_2_comparing_3_v_3_across_entire_90_second_gap.csv")
write_csv(df.sum3.10sec, "shuttlebox_data_october_4_2018_all_using_giveup_2_comparing_3_v_3_across_across_last_10_second_gap.csv")
write_csv(df.sum3.30sec, "shuttlebox_data_october_4_2018_all_using_giveup_2_comparing_3_v_3_across_across_last_30_second_gap.csv")
