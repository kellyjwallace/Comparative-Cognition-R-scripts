...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
#Does sex predict solving the barrier problem? THIS IS A PROBLEM BECAUSE TWO CATEGORICAL (need chi-squared)
...
p <- ggplot(df.full, aes(x=sex.y, y=detour_trial_1_barrier_problem_time_around_barrier_minus_time_to_contact_barrier)) + 
  ...
  geom_text(x=1, y=-10, label= glue::glue("p= {round(p_value_t,4)}"), size = 5, color = "gray27", fontface = 'italic') +
  labs(x= "Sex",y="Time to solve detour maze", size = 20, color = "gray39", fontface = 'italic') +
  scale_x_discrete(breaks=c('F','M'),labels=c("Female", "Male")) + theme(axis.text.y = element_text(size = 20)) + theme(axis.text.x = element_text(size = 20)) +
...
p <- ggplot(df.full, aes(x=sex.y, y=time_to_barrier_contact_minus_start_time)) + 
  ...
  scale_x_discrete(breaks=c(0, 1),labels=c("Females", "Males")) + theme(axis.text.y = element_text(size = 14)) + theme(axis.text.x = element_text(size = 14))
p
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
#Does Detour category (solved barrier problem versus did not solve barrier problem) predict numerosity performance over T30 to T50? MALES
...
p <- ggplot(df.full.males, aes(x=detour_trial_1_did_fish_solve_barrier_problem, y=average_performance_across_all_9_testing_trials_using_T30_to_T50_data)) + 
  geom_boxplot()+ stat_n_text() +
  ggtitle('Does Detour category (solved barrier problem versus did not solve 
          barrier problem) predict numerosity performance over T30 to T50? MALES') +
  geom_text(x=1, y=0.1, label= glue::glue("manually enter t-test p value from code
                                          p(wilcox) = {round(p_value_wilcox,4)}"), size = 6, color = "black")
p
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
#Does Detour category (solved barrier problem versus did not solve barrier problem) predict numerosity performance over T30 to T50? FEMALES
...
p <- ggplot(df.full.females, aes(x=detour_trial_1_did_fish_solve_barrier_problem, y=average_performance_across_all_9_testing_trials_using_T30_to_T50_data)) + 
  geom_boxplot()+ stat_n_text() +
  ggtitle('Does Detour category (solved barrier problem versus did not solve 
          barrier problem) predict numerosity performance over T30 to T50?females') +
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
#Does size/age predict detour latency to solve?
...
number <- df.full %>% filter(!is.na(detour_trial_1_barrier_problem_time_around_barrier_minus_time_to_contact_barrier)) %>% filter(!is.na(standard_length)) %>% nrow
p_value <- mod %>% broom::tidy() %>% filter(term != "(Intercept)") %>% select(p.value) %>% unlist
r_squared <- mod %>% broom::glance() %>% select(r.squared) %>% unlist
...
p + geom_point(size = 4) + 
  ...
  annotate("text", x=1800, y=30, label= glue::glue("p = {round(p_value,4)}, R-squared:  {round(r_squared,3)}
        n = {number}"), size = 6, color = "black") + 
  labs(x= "standard_length",y="detour_trial_1_barrier_problem_time_around_barrier_minus_time_to_contact_barrier") +
  ggtitle ("Does size/age predict detour latency to solve? SEXES COMBINED") +
  geom_smooth(method = "lm")
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
#Does size/age predict detour latency to solve?MALES
...
number <- df.full.males %>% filter(!is.na(detour_trial_1_barrier_problem_time_around_barrier_minus_time_to_contact_barrier)) %>% filter(!is.na(standard_length)) %>% nrow
p_value <- mod %>% broom::tidy() %>% filter(term != "(Intercept)") %>% select(p.value) %>% unlist
r_squared <- mod %>% broom::glance() %>% select(r.squared) %>% unlist
...
p + geom_point(size = 4) + 
  ...
  annotate("text", x=1800, y=30, label= glue::glue("p = {round(p_value,4)}, R-squared:  {round(r_squared,3)}
        n = {number}"), size = 6, color = "black") + 
  labs(x= "standard_length",y="detour_trial_1_barrier_problem_time_around_barrier_minus_time_to_contact_barrier") +
  ggtitle ("Does size/age predict detour latency to solve? MALES") +
  geom_smooth(method = "lm")
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
#Does size/age predict detour latency to solve?females
...
number <- df.full.females %>% filter(!is.na(detour_trial_1_barrier_problem_time_around_barrier_minus_time_to_contact_barrier)) %>% filter(!is.na(standard_length)) %>% nrow
p_value <- mod %>% broom::tidy() %>% filter(term != "(Intercept)") %>% select(p.value) %>% unlist
r_squared <- mod %>% broom::glance() %>% select(r.squared) %>% unlist
...
p + geom_point(size = 4) + 
  ...
  annotate("text", x=1800, y=30, label= glue::glue("p = {round(p_value,4)}, R-squared:  {round(r_squared,3)}
        n = {number}"), size = 6, color = "black") + 
  labs(x= "standard_length",y="detour_trial_1_barrier_problem_time_around_barrier_minus_time_to_contact_barrier") +
  ggtitle ("Does size/age predict detour latency to solve? females") +
  geom_smooth(method = "lm")
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
#Does size/age predict detour motivation?
mod<- lm(df.full$time_to_barrier_contact_minus_start_time ~ df.full$standard_length, na.rm = TRUE)
summary(mod)

number <- df.full %>% filter(!is.na(time_to_barrier_contact_minus_start_time)) %>% filter(!is.na(standard_length)) %>% nrow
p_value <- mod %>% broom::tidy() %>% filter(term != "(Intercept)") %>% select(p.value) %>% unlist
r_squared <- mod %>% broom::glance() %>% select(r.squared) %>% unlist
...
p + geom_point(size = 4) + 
  ...
  annotate("text", x=1800, y=30, label= glue::glue("p = {round(p_value,4)}, R-squared:  {round(r_squared,3)}
        n = {number}"), size = 6, color = "black") + 
  labs(x= "standard_length",y="time_to_barrier_contact_minus_start_time") +
  ggtitle ("Does size/age predict detour motivation? SEXES COMBINED") +
  geom_smooth(method = "lm")
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
#Does size/age predict detour motivation?MALES
mod<- lm(df.full.males$time_to_barrier_contact_minus_start_time ~ df.full.males$standard_length, na.rm = TRUE)
summary(mod)

number <- df.full.males %>% filter(!is.na(time_to_barrier_contact_minus_start_time)) %>% filter(!is.na(standard_length)) %>% nrow
p_value <- mod %>% broom::tidy() %>% filter(term != "(Intercept)") %>% select(p.value) %>% unlist
r_squared <- mod %>% broom::glance() %>% select(r.squared) %>% unlist
...
p + geom_point(size = 4) + 
  ...
  annotate("text", x=1800, y=30, label= glue::glue("p = {round(p_value,4)}, R-squared:  {round(r_squared,3)}
        n = {number}"), size = 6, color = "black") + 
  labs(x= "standard_length",y="time_to_barrier_contact_minus_start_time") +
  ggtitle ("Does size/age predict detour motivation? MALES") +
  geom_smooth(method = "lm")
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
#Does size/age predict detour motivation?females
mod<- lm(df.full.females$time_to_barrier_contact_minus_start_time ~ df.full.females$standard_length, na.rm = TRUE)
summary(mod)

number <- df.full.females %>% filter(!is.na(time_to_barrier_contact_minus_start_time)) %>% filter(!is.na(standard_length)) %>% nrow
p_value <- mod %>% broom::tidy() %>% filter(term != "(Intercept)") %>% select(p.value) %>% unlist
r_squared <- mod %>% broom::glance() %>% select(r.squared) %>% unlist
...
p + geom_point(size = 4) + 
  ...
  annotate("text", x=1800, y=30, label= glue::glue("p = {round(p_value,4)}, R-squared:  {round(r_squared,3)}
        n = {number}"), size = 6, color = "black") + 
  labs(x= "standard_length",y="time_to_barrier_contact_minus_start_time") +
  ggtitle ("Does size/age predict detour motivation? females") +
  geom_smooth(method = "lm")
