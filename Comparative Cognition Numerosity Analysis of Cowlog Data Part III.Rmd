...
test<-t.test(df.full$time_to_barrier_contact_minus_start_time~df.full$sex.x)
p_value_t<- test %>% broom::tidy() %>% select(p.value) %>% unlist
wilcox_test<-wilcox.test(df.full$time_to_barrier_contact_minus_start_time~df.full$sex.x)
p_value_wilcox<-wilcox_test$p.value

p <- ggplot(df.full, aes(x=sex.x, y=time_to_barrier_contact_minus_start_time)) + 
  geom_boxplot()+ stat_n_text() +
