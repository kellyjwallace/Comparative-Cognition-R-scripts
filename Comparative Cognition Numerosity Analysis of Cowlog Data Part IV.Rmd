...
#compare first 5 gaps to last 5 gaps
  giveup<- as.numeric(giveup)
  giveupminus4<-giveup - 4
  
  if(giveup > 9){
    df.full.first5gap<-df.full.gap %>% filter(is.inrange(presentation, range = c(1,5)))
    df.full.last5gap<-df.full.gap %>% filter(is.inrange(presentation, range = c(giveupminus4, giveup)))
    df.prop.first5gap<-df.prop.gap %>% filter(is.inrange(presentation, range = c(1,5)))
    df.prop.last5gap<-df.prop.gap %>% filter(is.inrange(presentation, range = c(giveupminus4, giveup)))
...
df.sum$presentation_when_fish_gave_up[i] <-giveup
...
#df.sum.10sec$presentation_when_fish_gave_up[i] <-giveup
...
df.sum.10sec$last10secdata_difference_avg_prop_correct_first_5_gaps_minus_last_5_gaps[i] <- comp_5v5_avg_prop_correct #so a more negative number
...
df.sum.10sec$last10secdata_binned_shuttlebox_slope_using_avg_dist_from_correct[i]<- binned_slope_avg_dist
df.sum.10sec$last10secdata_binned_shuttlebox_wilcox_test_using_avg_dist_from_correct[i]<-binned_wilcox_p_value_avg_dist
...
df.sum.10sec$last10secdata_binned_shuttlebox_slope_using_proportion_time_correct_during_gap[i]<- binned_slope_prop_correct
df.sum.10sec$last10secdata_binned_shuttlebox_wilcox_test_using_proportion_time_correct_during_gap[i]<-binned_wilcox_p_value_prop_correct
...
df.sum.10sec$last10secdata_binned_shuttlebox_average_proportion_of_time_on_correct_side_during_gaps[i]<- mean_binned_prop_time_correct_during_gap
#df.sum.10sec$binned_shuttlebox_average_proportion_of_time_on_the_correct_side_during_stimulus[i]<- mean_binned_prop_time_correct_during_stim
df.sum.10sec$last10secdata_binned_shuttlebox_proportion_of_time_on_correct_side_during_gaps_slope_across_presentations[i]<- gap_prop_binned_slope
#df.sum.10sec$binned_shuttlebox_proportion_of_time_on_correct_side_during_stimulus_slope_across_presentations[i]<- stim_prop_binned_slope
...
if(df.sum.10sec$last10secdata_binned_shuttlebox_wilcox_test_using_avg_dist_from_correct[i] <0.05){
    df.sum.10sec$last10secdata_binned_did_fish_solve_shuttlebox_using_avg_dist_from_correct[i]<- 1}
  else if(df.sum.10sec$last10secdata_binned_shuttlebox_wilcox_test_using_avg_dist_from_correct[i] >=0.05){
    df.sum.10sec$last10secdata_binned_did_fish_solve_shuttlebox_using_avg_dist_from_correct[i]<- 0}
  else{
    df.sum.10sec$last10secdata_binned_did_fish_solve_shuttlebox_using_avg_dist_from_correct[i]<- NA}
...
if(df.sum.10sec$last10secdata_shuttlebox_wilcox_test_using_proportion_time_correct_during_gap[i] <0.05){
    ...
    df.sum.10sec$last10secdata_did_fish_solve_shuttlebox_using_proportion_time_correct_during_gap[i]<- NA}

if(df.sum.10sec$last10secdata_binned_shuttlebox_wilcox_test_using_proportion_time_correct_during_gap[i] <0.05){
    df.sum.10sec$last10secdata_binned_did_fish_solve_shuttlebox_using_proportion_time_correct_during_gap[i]<- 1}
  else if(df.sum.10sec$last10secdata_binned_shuttlebox_wilcox_test_using_proportion_time_correct_during_gap[i] >=0.05){
    df.sum.10sec$last10secdata_binned_did_fish_solve_shuttlebox_using_proportion_time_correct_during_gap[i]<- 0}
  else{
    df.sum.10sec$last10secdata_binned_did_fish_solve_shuttlebox_using_proportion_time_correct_during_gap[i]<- NA}
...
else if(df.sum.10sec$last10secdata_difference_avg_prop_correct_first_5_gaps_minus_last_5_gaps[i]< -0.11){
  df.sum.10sec$last10secdata_was_difference_avg_prop_correct_first_5_gaps_minus_last_5_gaps_at_least_20_percent[i]<-1}
else{df.sum.10sec$last10secdata_was_difference_avg_prop_correct_first_5_gaps_minus_last_5_gaps_at_least_20_percent[i]<- 0}

#note 256 is 20% of 1280 so this is asking if their average distance decreased by 20%
...
else{df.sum.10sec$last10secdata_was_difference_avg_dist_from_correct_first_5_gaps_minus_last_5_gaps_at_least_20_percent[i]<- 0}
}
...
for(i in 1:length(file_list_filtered)){
  ...
  giveup <- strsplit(file_list_filtered[i],'_')[[1]][6] #this is the last presentation before the fish just camped out on one side for the rest of the trial. Kelly just eyeballed this using an individual graph for each fish's trial (distance from correct side). 
  giveup<-as.numeric(giveup)
  ...
  #filter out anything after giveup
  #get prop
  
  df.full<-df.full %>% filter(presentation <= giveup)
  df.full<-df.full %>% mutate(distance_from_correct = abs(correct_x_pixel - x))
  df.full.stim<-df.full %>% filter(type == 'stim')
  df.full.gap<-df.full %>% filter(type == 'gap')
...
#mean distance during stims (now only looking before giveup)
mean(df.mean$mean_distance_from_correct_during_stims)

#mean distance during gaps (now only looking before giveup)
mean(df.mean$mean_distance_from_correct_during_gaps)
#mean prop time correct during stims (now only looking before giveup)
mean(df.mean$mean_prop_time_correct_during_stim)
#mean prop time correct during gaps (now only looking before giveup)
