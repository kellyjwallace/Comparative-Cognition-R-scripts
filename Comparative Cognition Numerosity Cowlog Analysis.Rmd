...
```{r}
#4c. remove latency value if latency to enter correct or incorrect side = 0 i.e. fish was already there
#df <- filter(df, df$latency_enter_corr != 0)
#df <- filter(df, df$latency_enter_incorr != 0)
#NEED TO FIX THIS ONE MORE    
...
for(i in 1:nrow(df.sum)){
  df.sum$mean_of_median_performance_on_each_ratio_full_four_minutes_data[i] <- mean(c(df.sum$median_performance_on_0.50_ratio_using_full_4_minutes_data[i],df.sum$median_performance_on_0.67_ratio_using_full_4_minutes_data[i], df.sum$median_performance_on_0.75_ratio_using_full_4_minutes_data[i]))
   
  df.sum$mean_of_median_performance_on_each_ratio_T30_to_T50_data[i] <- mean(c(df.sum$median_performance_on_0.50_ratio_using_T30_to_T50_data[i],df.sum$median_performance_on_0.67_ratio_using_T30_to_T50_data[i], df.sum$median_performance_on_0.75_ratio_using_T30_to_T50_data[i]))  
}
...
write_csv(df.sum, "Numerosity Comparative Cognition Full Dataset March_19_2018.csv")
...
```{r}
#8a. Sort the import df (earlier spreadsheet) by trial ID
