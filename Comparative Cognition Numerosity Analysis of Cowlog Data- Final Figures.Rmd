...
#FIGURE 1
#A- HISTOGRAM: Shuttlebox learning category (did fish learn of screen time 50% within 10 cm 3 consecutive trials in a row before they gave up), number of learners vs nonlearners, colored by sex
#B- HISTOGRAM: numerical discrimination learning category (did fish have a median above 60), number of learners vs nonlearners, colored by sex
#C- HISTOGRAM: detour learning category (did fish successfully navigate around barrier), number of learners vs nonlearners, colored by sex

```{r, echo=FALSE, message=FALSE, warning=FALSE}
df.full.numhist<-df.full%>%filter(is.na(df.full$did_fish_have_a_median_above_60) == FALSE)
df.full.numhist$did_fish_have_a_median_above_60<-as.numeric(df.full.numhist$did_fish_have_a_median_above_60)
solvers = c(12,11)
nonsolvers = c(6,7)
numsolvesex<-data.frame(solvers,nonsolvers)
chisq.test(numsolvesex)

plot1A<- ggplot(df.full.numhist, aes(did_fish_have_a_median_above_60, fill = sex.y))+
  geom_histogram(binwidth = 0.5) + 
  labs(x= "numerical discrimination",y="number of individuals") +  
  scale_x_discrete(breaks=c(0, 1)) +
  scale_y_continuous(limits =c(0,30), breaks =seq(0,30,5)) +
  annotate("text", x=-0.5, y=15, label= glue::glue("Non-learners"), size = 7, color = "black", hjust = 0, fontface = 'italic') +
  annotate("text", x=0.66, y=25, label= glue::glue("Learners"), size = 7, color = "black", hjust = 0, fontface = 'italic') +
  theme(plot.title = element_text(hjust = 0)) +
  theme(text = element_text(size=20)) +
  annotate("text", x=-0.06, y=8.6, label= glue::glue("6"), size = 8, color = "white", hjust = 0)+
  annotate("text", x=-0.06, y=2, label= glue::glue("7"), size = 8, color = "white", hjust = 0) +
  annotate("text", x=0.9, y=13, label= glue::glue("12"), size = 8, color = "white", hjust = 0)+
  annotate("text", x=0.9, y=2, label= glue::glue("11"), size = 8, color = "white", hjust = 0) +
  ...
  #annotate("text", x=-0.8, y=6.1, parse = TRUE, label= glue::glue("{as.character(chisquaredtxt)}"), size = 5, color = "gray39", hjust = 0) + 
  #annotate("text", x=-0.7, y=5.9, label= glue::glue("= 0"), size = 6, color = "gray39", hjust = 0, fontface = 'italic') + 
...
df.full.dethist<-df.full%>%filter(is.na(df.full$detour_trial_1_did_fish_solve_barrier_problem) == FALSE)
df.full.dethist$detour_trial_1_did_fish_solve_barrier_problem<-as.numeric(df.full.dethist$detour_trial_1_did_fish_solve_barrier_problem)
detsolvers = c(14,13)
detnonsolvers = c(13,12)
detsolvesex<-data.frame(detsolvers,detnonsolvers)
chisq.test(detsolvesex)

plot1B<- ggplot(df.full.dethist, aes(detour_trial_1_did_fish_solve_barrier_problem, fill = sex.x))+
  geom_histogram(binwidth = 0.5) + 
  labs(x= "detour maze",y="number of individuals") +  
  scale_x_discrete(breaks=c(0, 1)) +
  scale_y_continuous(limits =c(0,30), breaks =seq(0,30,5)) +
  annotate("text", x=-0.5, y=27, label= glue::glue("Non-solvers"), size = 7, color = "black", hjust = 0, fontface = 'italic') +
  annotate("text", x=0.7, y=29, label= glue::glue("Solvers"), size = 7, color = "black", hjust = 0, fontface = 'italic') +
  theme(plot.title = element_text(hjust = 0)) +
  theme(text = element_text(size=20)) +
  annotate("text", x=-0.12, y=14, label= glue::glue("13"), size = 8, color = "white", hjust = 0)+
  annotate("text", x=-0.12, y=2, label= glue::glue("12"), size = 8, color = "white", hjust = 0) +
  annotate("text", x=0.88, y=15, label= glue::glue("14"), size = 8, color = "white", hjust = 0)+
  annotate("text", x=0.88, y=2, label= glue::glue("13"), size = 8, color = "white", hjust = 0) +
  scale_color_manual(values=c("maroon", "lightblue")) +
  theme(legend.position="left") + 
  scale_fill_manual(values=c("maroon", "lightblue"),name="Sex",labels=c("  Female", "  Male")) #+ 
  #annotate("text", x=-0.8, y=6.1, parse = TRUE, label= glue::glue("{as.character(chisquaredtxt)}"), size = 5, color = "gray39", hjust = 0) + 
...
df.full.shuthist<-df.full%>%filter(is.na(df.full$did_fish_learn_OF_SCREEN_TIME_50_percent_within_10_cm_3_consecutive) == FALSE)
df.full.shuthist$did_fish_learn_OF_SCREEN_TIME_50_percent_within_10_cm_3_consecutive<-as.numeric(df.full.shuthist$did_fish_learn_OF_SCREEN_TIME_50_percent_within_10_cm_3_consecutive)

df.full.shuthist%>%filter(sex.x == "M") %>%filter(did_fish_learn_OF_SCREEN_TIME_50_percent_within_10_cm_3_consecutive == 1) %>%filter(did_fish_participate_criterion7 == 1) %>%nrow
df.full.shuthist%>%filter(sex.x == "F") %>%filter(did_fish_learn_OF_SCREEN_TIME_50_percent_within_10_cm_3_consecutive == 1) %>%filter(did_fish_participate_criterion7 == 1)%>%nrow


df.full.shuthist%>%filter(sex.x == "M") %>%filter(did_fish_learn_OF_SCREEN_TIME_50_percent_within_10_cm_3_consecutive == 0) %>%filter(did_fish_participate_criterion7 == 1)%>%nrow
df.full.shuthist%>%filter(sex.x == "F") %>%filter(did_fish_learn_OF_SCREEN_TIME_50_percent_within_10_cm_3_consecutive == 0) %>%filter(did_fish_participate_criterion7 == 1)%>%nrow

shutsolvers = c(0,7)
shutnonsolvers = c(17,12)
shutsolvesex<-data.frame(shutsolvers,shutnonsolvers)
chisq.test(shutsolvesex)

plot1C<- ggplot(df.full.shuthist, aes(detour_trial_1_did_fish_solve_barrier_problem, fill = sex.x))+
  geom_histogram(binwidth = 0.5) + 
  labs(x= "detour maze",y="number of individuals") +  
  scale_x_discrete(breaks=c(0, 1)) +
  scale_y_continuous(limits =c(0,30), breaks =seq(0,30,5)) +
...
#FIGURE 2
#A- SCATTER PLOT: numerosity reaction time by average performance, FEMALES
#B- SCATTER PLOT:numerosity exploration by average performance, FEMALES
#C- SCATTER PLOT:numerosity avtivity by average performance, FEMALES
#D- SCATTER PLOT:numerosity reaction time by average performance, MALES
#E- SCATTER PLOT:numerosity exploration by average performance, MALES
#F- SCATTER PLOT:numerosity activity by average performance, MALES
#G- BOXPLOT: numerosity reaction time across the four groups: male learners, male nonlearners, female learners, female nonlearners
#H- BOXPLOT: numerosity exploration across the four groups: male learners, male nonlearners, female learners, female nonlearners
#I- BOXPLOT: numerosity activity across the four groups: male learners, male nonlearners, female learners, female nonlearners


#FIGURE 2 ALTERNATIVE
#A- SCATTER PLOT: numerosity reaction time by average performance, colored by sex
#B- SCATTER PLOT: numerosity exploration by average performance, colored by sex
#C- SCATTER PLOT: numerosity activity time by average performance, colored by sex



#FIGURE 3
#A- BOXPLOT: detour learning category by average numerosity performnace, sexes combined
#B- BOXPLOT: detour learning category by average numerosity performnace, MALES
#C- BOXPLOT: detour learning category by average numerosity performnace, FEMALES
...
plot3 <- ggplot(df.full, aes(x=detour_trial_1_did_fish_solve_barrier_problem, y=time_to_barrier_contact_minus_start_time)) + 
  geom_boxplot(lwd=1, outlier.colour = "black")+ stat_n_text(size = 5) + theme_minimal() +
  ...
  scale_x_discrete(breaks=c(0, 1),labels=c("Non-solvers", "Solvers")) + theme(axis.text.y = element_text(size = 15)) + theme(axis.text.x = element_text(size = 15)) +
...
plot1 <- ggplot(df.full, aes(x=detour_trial_1_did_fish_solve_barrier_problem, y=average_performance_across_all_9_testing_trials_using_T30_to_T50_data)) + 
  geom_boxplot(lwd=1)+ stat_n_text(size = 5) + theme_minimal() +
 ggtitle ('SEXES COMBINED') +
  labs(x= "Detour Learning Category",y="Average numerosity performance \n (1st 20 seconds only)") +
  geom_text(x=1, y=0.08, label= glue::glue("p= {round(p_value_t,3)}"), size = 5, color = "gray39") +  scale_x_discrete(breaks=c(0, 1),labels=c("Non-solvers", "Solvers")) +
  theme(axis.text.y = element_text(size = 17)) + theme(axis.text.x = element_text(size = 17)) +
  theme(axis.title = element_text(size = 17))
plot1
...
plot1 <- ggplot(df.full.females, aes(x=detour_trial_1_did_fish_solve_barrier_problem, y=average_performance_across_all_9_testing_trials_using_T30_to_T50_data)) + 
  ...
  ggtitle ('FEMALES') +
  labs(x= "Detour Learning Category",y="Average numerosity performance \n (1st 20 seconds only)") +
  geom_text(x=1, y=0.3, label= glue::glue("p= {round(p_value_t,3)}"), size = 5, color = "gray39") +  scale_x_discrete(breaks=c(0, 1),labels=c("Non-Learners", "Learners")) +
...
shapiro.test(df.full$average_performance_across_all_9_testing_trials_using_T30_to_T50_data)
...
test<-t.test(df.full$average_number_of_entries_full_4_minutes~df$did_fish_have_a_median_above_60)
p_value_t<- test %>% broom::tidy() %>% select(p.value) %>% unlist
...
plot1 <- ggplot(df.full.males, aes(average_number_of_entries_full_4_minutes, average_performance_across_all_9_testing_trials_using_T30_to_T50_data)) + geom_point(size = 4, color = "orange") + 
  theme(text = element_text(size=10)) + 
...
plot2 <- ggplot(df.full.males, aes(average_reaction_time, average_performance_across_all_9_testing_trials_using_T30_to_T50_data)) + geom_point(size = 4, color = "orange") + 
  theme(text = element_text(size=10)) + 
  annotate("text", x=0.2, y=0.33, parse = TRUE, label= glue::glue("{as.character(rsquaredtxt)}"), size = 5, color = "gray39", hjust = 0) + 
  annotate("text", x=1.3, y=0.32, label= glue::glue("= {round(r_squared,4)}"), size = 5, color = "gray39", hjust = 0) + 
  annotate("text", x=0.2, y=0.25, label= glue::glue("p = {round(p_value,4)}
n = {number}"), size = 5, color = "gray39", hjust = 0) + 
  labs(x= "Average reaction time",y="Average numerosity performance \n (1st 20 seconds only)") + 
  ggtitle ("males") +
...
plot3 <- ggplot(df.full.males, aes(unique_zones_entered_full_4_minutes, average_performance_across_all_9_testing_trials_using_T30_to_T50_data)) + geom_point(size = 4, color = "orange") + 
  theme(text = element_text(size=10)) + 
  annotate("text", x=0.82, y=0.35, parse = TRUE, label= glue::glue("{as.character(rsquaredtxt)}"), size = 5, color = "gray39", hjust = 0) + 
  annotate("text", x=0.832, y=0.345, label= glue::glue("= {round(r_squared,4)}"), size = 5, color = "gray39", hjust = 0) + 
  annotate("text", x=0.82, y=0.25, label= glue::glue("p = {round(p_value,4)}
